...
////////////////////////////////////////////////////////////
loginHandler(event){event.preventDefault();if(!this.checkValidity())return false;this.statusbar.status="loading";const body=Object.assign({},{email:this.emailInput.value,password:this.passwordInput.value},JSON.parse(this.postbody));return fetch(this.posturl,{method:"POST",redirect:"follow",credentials:"include",headers:new Headers({"Content-Type":"application/json"}),body:JSON.stringify(body)}).then(resp=>resp.json()).then(resp=>{if(resp.error)throw resp;return this._loginSuccess(resp)}).catch(resp=>this._loginError(resp))}_loginSuccess(resp){bounce(this.domNode,this.dispatchEvent.bind(this,"login/success",resp))}_loginError(resp){this.dispatchEvent("login/error",resp);this.errorHandler()}checkValidity(){return[this.emailInput.checkValidity(),this.passwordInput.checkValidity()].every(check=>check)}openHandler(event,target){event.preventDefault();bounce(event.target,this._location.bind(this,event.target.href,target))}_location(href,target){this.dispatchEvent("login/location",{href,target})}errorHandler(){this.writeMessageError=this.writeMessageError.bind(this);this.statusbar.status="alert";this.domNode.addEventListener("animationend",this.writeMessageError);this.domNode.classList.add("alert")}writeMessageError(){this.domNode.removeEventListener("animationend",this.writeMessageError);this.messageNode.innerHTML="";const errorMsg=html`
      <ul>
        <li>
          <a
